...
import numpy as np
from smoothness_metrics import sparc, dimensionless_jerk, log_dimensionless_jerk

class SmoothnessMeasurement:
    
    def __init__(self, qtm_obj, object, marker, segment_start, segment_end, 
...
def get_mean_sparc_for_all_segments(qtm_obj, object, marker, segment_size=3, segment_margin=8, sample_freq=500, padlevel=4, freq_cutoff=5, amp_th=0.05):
    scores = []
    total_size = len(qtm_obj.data[object, marker, 0, :])
    print(f"total size: {total_size} ({total_size/qtm_obj.frame_rate})")
    for segment_start in np.arange(segment_margin, total_size - segment_margin, segment_size):
        smoothness_comparator = SmoothnessMeasurement(qtm_obj, object, marker, segment_start, segment_start+segment_size)
        if segment_start + segment_size > total_size - segment_margin:
            break
        try:
            if smoothness_comparator.speed_profile.shape[0] == 0:
                continue
            scores.append(smoothness_comparator.get_sparc()[0])
        except ZeroDivisionError:
            print(f"got zero division error for segment {segment_start}-{segment_start+segment_size}")
            break
    return np.mean(scores)

def get_mean_jerk_for_all_segments(qtm_obj, object, marker, segment_size=3, segment_margin=8, sample_freq=500, padlevel=4, freq_cutoff=5, amp_th=0.05):
    scores = []
    total_size = len(qtm_obj.data[object, marker, 0, :]) 
    for segment_start in np.arange(segment_margin, total_size - segment_margin, segment_size):
        smoothness_comparator = SmoothnessMeasurement(qtm_obj, object, marker, segment_start, segment_start+segment_size)
        ...
        try:
            if smoothness_comparator.speed_profile.shape[0] == 0:
                continue
            scores.append(smoothness_comparator.get_dimensionless_jerk())
        except ZeroDivisionError:
            break
    return np.mean(scores)